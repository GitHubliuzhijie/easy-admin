...
.v-color-trigger {
  padding: 4px;
  border-radius: 6px;
  border: var(--easy-border, 1px solid #dfe1e5);
  background: var(--easy-color-bg-container, #f1f3f4);
  width: 32px;
  height: 32px;
  cursor: pointer;
  transition: all 0.2s var(--easy-motion-ease-in-out);
}
.v-color-trigger-active,
.v-color-trigger:hover {
  border-color: var(--easy-color-primary-hover, #3e74fd);
}
.v-color-trigger-disabled,
.v-color-trigger-disabled:hover {
  cursor: not-allowed;
  border-color: var(--easy-border-color, #dfe1e5);
  background-color: var(--easy-color-fill-tertiary);
}
