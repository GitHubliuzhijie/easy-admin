  ...
  <div class="v-reservation-panel">
      ...
      >
        <div
          ...
          :style="{
            ...
            width: `${col.width}px`,
            borderBottom: !isScrolledToTop ? 'var(--easy-border, 1px solid #ddd)' : 'none',
          }"
        >
    ...
    <div
      ...
      :style="{
        ...
        width: `${gridContainerWidth}px`,
        transform: `translateX(${props.leftFixedWidth}px)`,
        borderTop: props.resources.length === 0 ? 'var(--easy-border, 1px solid #ddd)' : 'none',
      }"
      @scroll="handleScroll"
      @wheel="handleContainerWheel"
    >
...
.v-reservation-panel {
  ...
  height: 100%;
  position: relative;
  border-bottom: var(--easy-border, 1px solid #ddd);
  border-right: var(--easy-border, 1px solid #ddd);
}
...
.v-fixed-header-container {
  border-top: var(--easy-border, 1px solid #ddd);
  overflow: hidden;
}
.v-fixed-header {
  position: relative;
...
.v-fixed-header-col {
  ...
  height: 100%;
  border-left: var(--easy-border, 1px solid #ddd);
  background-color: var(--easy-color-bg-container);
}
...
.v-y-scrollbar-mask {
  background: var(--easy-scrollbar-bg, #2c2c2c);
  position: absolute;
  right: 0;
  bottom: 0;
  z-index: 20;
...
.virtual-grid-cell-disabled {
  background: var(--easy-color-fill-tertiary, #f9f9f9);
}
/* 时间线 */
.v-time-line {
  position: absolute;
  left: 0;
  height: 1px;
  background: var(--easy-primary-color, #3e74fd);
  z-index: 1;
}
</style>
